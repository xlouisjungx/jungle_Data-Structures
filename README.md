...
* Binary Search Tree (BST) - 이진 탐색 트리
	...
	- 프로그래밍에서 가장 중요한 자료구조 중 하나이며, 효율적인 데이터 검색, 삽입, 삭제룰 가능하게 하며 다양한 알고리즘과 시스템에서 사용된다.
	- 그럼 이진 탐색 트리란? : 이진 트리의 한 종류로, 데이터를 정렬된 상태로 저장하며 효율적인 검색과 관리가 가능하도록 설계된 자료구조.

	- 기본 구조
		- 각 노드는 최대 두 개의 자식을 가진다.
		- 왼쪽 서브트리에는 부모 노드보다 작거나 같은 값이 저장된다.
		- 오른쪽 서브트리에는 부모 노드보다 큰 값이 저장된다.

	- 특징
		- 노드를 중위 순회하면 정렬된 데이터가 출력된다.
